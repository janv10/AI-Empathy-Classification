...
def knn(train, test, k):
    ...
    if k > 0 and k <= len(train):
        ...
        for te in range(len(test)):
            ...
            for tr in range(len(train)):
                sums = 0
                for c in range(len(train[0])-1):
                    sums += abs(test[te][c] - train[tr][c])
                singleres.append((sums, tr, train[tr][16]))
    ...
    for i in range(len(inter)):
        if inter[i] == True:
            rcount += 1
        count += 1
    print("KNN using 16 most correlated variables run with k = " + str(k) + " yielded a testing accuracy of: " + str(rcount / count))
